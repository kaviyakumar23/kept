...
7. **Honesty framing.** Slack is the real, live surface. Work items (Linear/Jira) + proof sources (LaunchDarkly flags, Atlassian Statuspage) are simulated via an in-process MCP server for the demo, with real API skeletons; **GitHub Actions is a genuine live proof source.** Never imply a certified live integration for a simulated one — the honesty is a credibility beat.
...
- **Integrations:** `src/integrations/*` — `mcp` (work items + generic `query()` + simulated servers), linear/jira, proof adapters (github **live**; launchdarkly/statuspage skeleton), roadmapPostgres.
